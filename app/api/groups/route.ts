...
export async function POST(request: Request) {
    const { name, description, creator_id, image_url, cover_url, is_private } = await request.json()
    ...
    // Auto-join creator with role='creator'
    await fetch(`${SUPA_URL}/rest/v1/feed_group_members`, {
        method: 'POST',
        headers: { ...h, 'Prefer': 'return=minimal' },
        body: JSON.stringify({ group_id: g.id, user_id: creator_id, role: 'creator' })
    })
